...
=======
    ...
    vec3 pos = vec3(uv * 3.0, uTime + abs((1.1 - uCursorX)) + abs(1.1 - uCursorX)  * 0.1);

    // pos = step(0.2, pos);

    float myNoise = pnoise(pos, vec3(0.0));


    float test = smoothstep(0.2, 0.21, myNoise);

    vec3 color = vec3( step(0.0, myNoise));
        //  color = vec3( step(0.1, myNoise));


         color = vec3( test * (0.6 + uCursorX), test * (0.4 + uCursorY + uv.y), test * 1.0);
>>>>>>> Stashed changes
